...
static std::vector<FinishedBook> locally_finished_books;
...
static void request_ai_recommendation(const std::string &user_prompt);
...
static void request_ai_recommendation(const std::string &user_prompt) {
  ...
  std::thread([user_prompt]() {
    httplib::Client cli("127.0.0.1", 8000);
    cli.set_connection_timeout(30, 0);   // 30 seconds to connect
    cli.set_read_timeout(300, 0);        // 5 minutes to wait for LLM response
